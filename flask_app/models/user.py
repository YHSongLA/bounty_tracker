...
from flask_app.controllers.bounties import Bounty
...
class User:
    ...
    @classmethod
    def get_one_with_bounty(cls,data:dict) -> object:
        query  = "SELECT * FROM users JOIN bounties ON bounties.user_id = users.id WHERE users.id = %(id)s;"
        ...
        for result in results:
            sub_data = {
                ...
                'created_at': result['bounties.created_at'],
                'updated_at': result['bounties.updated_at']
            }
            user.bounties.append(Bounty(sub_data))
        return user
